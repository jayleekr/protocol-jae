...
### 입력
- GitHub 저장소 분석 결과 (jae-repo-analyzer)
- 코드 메트릭 데이터 (jae-code-metrics-collector)
- 과거 평가 데이터 (트렌드 분석용)
- 벤치마크 데이터 (업계/조직 표준)
...
### 다음 단계 에이전트
- **jae-improvement-strategist**: 개선 전략 수립
- **jae-repo-insights-orchestrator**: 최종 통합 보고서
...
```bash
# 프로젝트 건강도 평가 실행
./temp_hooks/commands/agents/jae-project-health-evaluator/run.sh \
