...
### 입력
- 데이터베이스 설계 명세서 (velocity-x-data-architect로부터)
- 비즈니스 요구사항 및 데이터 정책
- 성능 요구사항 및 확장성 목표
- 보안 정책 및 컴플라이언스 요구사항
...
### 다음 단계 에이전트
- **velocity-x-backend-builder**: 데이터베이스 연동
- **velocity-x-integration-builder**: 외부 시스템 데이터 통합
- **velocity-x-performance-optimizer**: 데이터베이스 성능 최적화
- **velocity-x-security-scanner**: 데이터베이스 보안 검사

## 설정 요구사항

```yaml
agent_config:
  name: velocity-x-database-builder
