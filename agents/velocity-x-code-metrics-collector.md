...
### 연계 에이전트
- **velocity-x-repo-analyzer**: GitHub 메타데이터와 연계
- **velocity-x-project-health-evaluator**: 종합 건강도 평가 지원
- **velocity-x-improvement-strategist**: 개선 전략 수립 지원
...
```bash
# 코드 메트릭 수집 실행
./temp_hooks/commands/agents/velocity-x-code-metrics-collector/run.sh \
  --project-path "./src" \
  --include-tests \
