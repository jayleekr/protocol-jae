...
### 연계 에이전트
- **velocity-x-meta-coordinator**: 품질 기준 조율 및 의사결정
- **velocity-x-process-optimizer**: 품질 프로세스 개선 협업
- **velocity-x-knowledge-curator**: 품질 가이드라인 문서화
- **모든 VELOCITY-X 에이전트**: 품질 기준 적용 및 피드백

## 설정 요구사항

```yaml
agent_config:
  name: velocity-x-quality-guardian
