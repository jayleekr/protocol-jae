...
### 입력
- 프로젝트 건강도 평가 결과 (velocity-x-project-health-evaluator)
- 리스크 평가 보고서
- 팀 역량 및 리소스 정보
- 조직 목표 및 제약사항
...
### 연계 에이전트
- **velocity-x-repo-insights-orchestrator**: 최종 통합 보고서 지원
- **velocity-x-documentation-scribe**: 전략 문서화 지원
...
```bash
# 개선 전략 수립 실행
./temp_hooks/commands/agents/velocity-x-improvement-strategist/run.sh \
