...
### 다음 단계 에이전트
- **velocity-x-security-scanner**: 보안 테스트 통합
- **velocity-x-performance-optimizer**: 성능 테스트 결과 분석
- **velocity-x-quality-guardian**: 품질 게이트 검증
- **velocity-x-deployment-manager**: 테스트 통과 후 배포 진행

## 설정 요구사항

```yaml
agent_config:
  name: velocity-x-test-automator
