# JAE-REPO-INSIGHTS-ORCHESTRATOR
...
### 하위 에이전트 관리
1. **jae-repo-analyzer**: GitHub 메타데이터 수집
2. **jae-code-metrics-collector**: 코드 품질 정량화
3. **jae-project-health-evaluator**: 종합 건강도 평가
4. **jae-improvement-strategist**: 개선 전략 수립
...
def create_web_dashboard():
    ...
    st.set_page_config(
        page_title="JAE Repository Insights",
        page_icon="📊",
        layout="wide"
    )
    
    st.title("🔍 JAE Repository Insights Dashboard")
...
```bash
# 기본 분석
./temp_hooks/commands/agents/jae-repo-insights-orchestrator/run.sh \
