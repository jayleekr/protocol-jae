...
### 입력
- CI/CD 파이프라인 결과 (velocity-x-cicd-builder로부터)
- 컨테이너 이미지 또는 빌드 아티팩트
- 환경별 설정 파일
- 배포 승인 및 일정
...
### 다음 단계 에이전트
- **velocity-x-ops-monitor**: 배포 후 모니터링 시작
- **velocity-x-progress-tracker**: 배포 진행 상황 추적
- **velocity-x-stakeholder-communicator**: 배포 결과 공유
...
```yaml
agent_config:
  name: velocity-x-deployment-manager
