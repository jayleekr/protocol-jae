...
JAE의 내장 에이전트들이 강력한 기능을 제공하지만, 진정한 마법은 특정 도메인, 도구, 워크플로우 요구사항에 맞춘 사용자 정의 에이전트를 생성할 때 일어납니다. 이 장에서는 JAE 생태계와 원활하게 통합되는 사용자 정의 에이전트를 개발, 테스트, 배포하는 포괄적인 가이드를 제공합니다.

이 장을 마치면 다음을 이해하게 됩니다:
- JAE 에이전트 아키텍처와 개발 패턴
...
모든 JAE 에이전트는 일관된 아키텍처 패턴을 따릅니다:
...
일관된 구현을 위해 JAE의 에이전트 프레임워크를 사용하세요:
...
class AgentMonitoringSystem:
    ...
    def create_monitoring_dashboard(self, agents: List[JAEAgent]) -> Dashboard:
        ...
        dashboard = Dashboard("JAE 에이전트 모니터링")
...
사용자 정의 에이전트 개발은 특정 요구사항을 충족하기 위해 JAE의 기능을 확장할 수 있게 해주는 강력한 기능입니다. 신중한 설계, 견고한 구현, 포괄적인 테스트, 적절한 배포 관행을 통해 JAE 생태계와 원활하게 통합되는 에이전트를 생성할 수 있습니다.

### 주요 포인트

✅ **구조화된 개발**: 일관성과 상호 운용성을 위해 JAE의 에이전트 아키텍처 패턴 따르기
...
2. **구현**: JAE 패턴을 따라 설계한 에이전트의 기본 버전 구현

3. **테스트**: 유닛, 통합, 성능 테스트를 포함한 에이전트의 포괄적인 테스트 스위트 생성

4. **통합**: 기존 JAE 워크플로우와 에이전트 통합
...
- [JAE 에이전트 API 레퍼런스](appendix-api-reference.md)
